...
func setup(owner: Node3D) -> void:
	...
	# 左上对话卡:统一 UIKit 卡片区,说话人做标题行。
	var card_ui := UIKit.make_card(layer)
	dialogue_card = card_ui.body
	card_title = card_ui.title
	card_panel = card_ui.panel
	# 底部中央按钮行,与其他系统共用同一锚位。
	dialogue_button = UIKit.make_button("E  交谈")
	dialogue_button.custom_minimum_size = Vector2(260, UIKit.BTN_HEIGHT)
	dialogue_button.focus_mode = Control.FOCUS_NONE
	dialogue_button.pressed.connect(_interact)
	UIKit.anchor_button_row(dialogue_button)
...
func set_active(value: bool) -> void:
	...
	set_process(value)
	if card_panel:
		card_panel.visible = value and active_id != ""
	if dialogue_button:
		dialogue_button.visible = false
...
func _offer(key: String, body: String, distance: float, holding := false) -> void:
	if router:
		router.offer("keeper", body, distance, key, 0, holding)
...
func _process(delta: float) -> void:
	...
	dialogue_button.visible = active_id != "" and _can_act()
	if active_id != "":
...
func _show_line() -> void:
	...
	card_title.text = speaker
	dialogue_card.text = line
...
func _update_dialogue_prompt() -> void:
	var data: Dictionary = npc_groups[active_id].data
	_offer("", "%s · %d / %d" % [String(data.names[0]), active_line + 1, data.lines.size()], 0.0, true)
